...
module.exports = {
    async pegaTudoAtrativo(req, res) {
        
        const tudoAtrativo = await Atrativo.findAll();

        return res.status(200).send(tudoAtrativo);
    },
    ...
    async pegaAtrativoPorLocalTotal (req, res) {
        ...
        for (let i=0; i<=(databaseAtrativo.length-1) ; i++) {
            if (databaseAtrativo[i].municipio == municipio ) {
                listaAtrativosLocais.push(databaseAtrativo[i]);
            }
        }
